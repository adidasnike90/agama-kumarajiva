...
- **V3**：法义读本——`book_v3/` · `make book-v3`（见 [V3_DHARMA_READER.md](V3_DHARMA_READER.md)；仅正文＋今译＋附注）
- **体例定位**：V1／V2 为研究译注本（五层对照）；V3 为通读删定本
- **副题（建议）**：Saṃyukta Āgama — A Kumarajiva-Style Research Translation
- **性质**：数字人文意义上的**文体实验译本** + **文献对照本**（V1／V2）及**法义通读本**（V3）；**不是**历史文献，**不声称**鸠摩罗什曾译《杂阿含》。
...
1. **不**改写或取代 **V1／V2** 研究译注本；**V3** 为同系列**法义读本**（通读熔文，网站可切换），见 [V3_DHARMA_READER.md](V3_DHARMA_READER.md)；
2. **不**引入后期大乘／禅语汇（见 `glossary/forbidden_mahayana.txt`）；
3. **不**将重建经伪装为底本全文；
4. **不**声称学术重排或通读熔文即「恢复原典」或罗什曾译阿含；
5. **不**在 V3 正文使用「序分／正宗分／流通分」科判标目（可读性优先；开—正—结精神可暗用）。
## 质量门禁（成书数据）
