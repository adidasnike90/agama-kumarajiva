...
\begin{description}[style=nextline,leftmargin=3.2em]
  ...
  各经【底本·求那跋陀罗译】即据此出；经题、卷次、品目（相应）亦依大正本经目框架排列。
  \item[卷次与品目]
  五十卷分界见传统《杂阿含》卷目；品目名如「阴相应」「见相应」同大正次序。
  本书经号为\textbf{该品内序号}，如阴相应（一）即全书第 1 经。
  \item[底本地位]
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{定位}：依大正经目确定卷、品、经号；载入汉译底本全文。
...
\noindent 本书为\textbf{数字人文与人工审读结合}之首版研究底本，欢迎依底本、平行与开放数据勘误。
...
\begin{itemize}
  \item \textbf{不}自动重排大正本经序错简（v1 从传统卷目出）。
...
\begin{description}[style=nextline,leftmargin=3.2em]
  ...
  \item[平行表]
  SuttaCentral  curated 平行关系为参考数据；本书据以配对，不声称原创发现。
\end{description}
...
\noindent 经号与平行编号须与所引各经【平行】栏一致。示例：
